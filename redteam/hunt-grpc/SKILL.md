...
- **`triage-validation`** — Apply the Status-Code Discrimination Gate: gRPC responses with `grpc-status: 0` (OK) are confirmed findings; `Unauthenticated (16)` / `PermissionDenied (7)` mean auth works — close the candidate. Never claim "no auth" from a non-empty response body that may be an error frame.
