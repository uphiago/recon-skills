...
|--------|----------|-----------|---------|-------|-------|------|
| ecommerce-wine.com | ✅ (register → upload → exec) | ❌ | ✅ | ✅ | 10+8+7=25 | #1 |
| biglots staging | ❌ | ✅ (15 endpoints) | ✅ | ❌ | 9+8=17 | #2 |
| mattress-retailer.com | 🔄 (needs brute 1st) | ❌ | ✅ | ✅ (multicall) | 8+7=15 | #3 |

## Key Insight

**Single-vector exploitation > multi-step chain.** A target with a 1-step RCE path (ecommerce-wine.com: register → upload → exec) is always higher priority than a target requiring a 4-step chain (phish → steal creds → login → upload → RCE), even if the latter has more total findings.
